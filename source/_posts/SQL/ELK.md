...
- 插入文档：PUT、POST /index_name/type_name/_doc/{id}，请求体为 JSON 格式的数据,uri中必须要有$_doc$
...
- 更新文档：POST /index_name/type_name/_update/{id}，请求体为 JSON 格式的数据,一定要有$doc$
...
| **部分更新**             | `POST` | `/<index>/_update/<id>`     | **用户明确指定** | **失败** (404 Not Found) | **更新** 指定字段       | 修改文档的特定部分（如计数器、状态标记、描述） | **部分更新** |
 ...
 1. 文档操作类：
    | **关键词**        | **作用**                                         | **示例**                                |
    |-------------------|-------------------------------------------------|----------------------------------------|
    | `_doc`            | 基础文档操作端点                                | `PUT /index/_doc/1`                    |
